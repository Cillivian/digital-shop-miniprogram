...
Page({
  ...
  onShareAppMessage() {
    ...
    const title = `这里有一个好宝贝,${detail.name||''}`;
  ...
  addCart(){
    ...
    let item=cart.find(e=>e._id==this.data.id)
    if(item){
      wx.showModal({
        title: '注意',
        content: '改商品已存在购物车'
      ...
      let data = Object.assign(this.data.detail, {
        number: 1,
        checked:true,
      })
      cart.push(data)
  ...
  addBuy(){
    let item = Object.assign(this.data.detail,{number:1,checked:true}) 
    let data={
      goods: [item],
      total: this.data.detail.price
    }
