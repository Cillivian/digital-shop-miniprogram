...
async function getRecord(event) {
  ...
  var pageSize = 20;
  const _ = db.command;
  var dbret = await db.collection('goods').where(_.and([{
    title: db.RegExp({
      regexp: '.*' + keyword,
      options: 'i',
    })
...
async function getAllRecord(event) {
  ...
  var dbret = await db.collection('goods').where(_.and(options)).orderBy('createTime', 'desc').skip(currentPage * pageSize).limit(pageSize).get();
  //console.log(dbret);
  var list = dbret.data;
  if (list && list.length) {
    for (var i = 0; i < list.length; i++) {
...
async function delRecord(event) {
  ...
  const dbret = await db.collection('goods').where({
    _id: id,
    _openid: openid
  }).update({
